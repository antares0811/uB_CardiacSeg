...
def test_single_volume(image, label, model, classes, patch_size=[256, 256, 16]):
    image, label = image.squeeze(0).cpu().detach(
    ).numpy(), label.squeeze(0).cpu().detach().numpy()
    image = np.transpose(image, (1, 2, 0))
    
    x, y, z = image.shape
    image = zoom(image, (patch_size[0] / x, patch_size[1] / y, patch_size[2] / z), order=0)
    ...
    with torch.no_grad():
        ...
        prediction = zoom(out, (x / patch_size[0], y / patch_size[1], z / patch_size[2]), order=0)
    
    prediction = prediction.astype(int)
    prediction[prediction >= classes] = classes - 1
