...
def getLargestCC(segmentation):
    ...
    if labels.max() == 0:
        return segmentation
    assert( labels.max() != 0 ), labels.max() # assume at least 1 CC
    largestCC = labels == np.argmax(np.bincount(labels.flat)[1:])+1
    return largestCC
...
# 3. validate --- LA
def var_all_case_LA(model, base_dir, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4):
    d_path = os.path.join(base_dir, "test.list")
    with open(d_path, 'r') as f:
        image_list = f.readlines()
    image_list = ["./data/LA/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
    # image_list = [item.replace('\n', '') for item in image_list]
    # image_list = [os.path.join(base_dir, item, "mri_norm2.h5") for item in image_list]
...
# 4. validate --- Pancrease
def var_all_case_Pancrease(model, base_dir, num_classes, patch_size=(96, 96, 96), stride_xy=16, stride_z=16, flag_nms=False):
    d_path = os.path.join(base_dir, "eval.list")
    with open(d_path, 'r') as f:
        image_list = f.readlines()
    # image_list = ["../../data/LA/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
    image_list = [item.replace('\n', '') for item in image_list]
    image_list = [os.path.join(base_dir, "data", f"{item}_norm.h5") for item in image_list]
    ...
    for image_path in loader:
        ...
        label = h5f['label'][:]
        prediction, score_map = test_single_case(model, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
        # prediction, score_map = test_single_case_pancrease(model, image, stride_xy, stride_z, patch_size, num_classes=num_classes)
        if flag_nms:
