...
class BaseDataSets(Dataset):
    def __init__(self, base_dir=None, split='train', num_d=2, num=None, transform=None):
        self._base_dir = base_dir
        self.sample_list = []
        self.split = split
        self.transform = transform
        self.num_d = num_d
        if self.split == 'train' and num_d==2:
            with open(self._base_dir + '/train_slices.list', 'r') as f1:
                self.sample_list = f1.readlines()
            self.sample_list = [item.replace('\n', '') for item in self.sample_list]
        elif self.split == 'train' and num_d==3:
    ...
    def __getitem__(self, idx):
        ...
        if self.num_d==2:
            ...
            list_slice = glob.glob(self._base_dir + "/data/{}*.h5".format(case))
            image = []
            label = []
            for slice in list_slice:
                h5f = h5py.File(slice, 'r')
                image.append(h5f['image'][:])
...
class BaseDataSets_25d(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.split == "train":
            ...
            num_slice = re.findall(r'\d+\.?\d*', case)[-1]

            if num_slice == '1':
                image0 = copy.deepcopy(image1)
            else:
                case0 = case.replace('slice_' + num_slice, 'slice_' + str(int(num_slice)-1))
                image0 = h5py.File(self._base_dir + "/data/{}.h5".format(case0), 'r')['image'][:]
            
            case2 = case.replace('slice_' + num_slice, 'slice_' + str(int(num_slice)+1))
            if not os.path.exists(self._base_dir + "/data/{}.h5".format(case2)):
                image2 = copy.deepcopy(image1)
            else:
                image2 = h5py.File(self._base_dir + "/data/{}.h5".format(case2), 'r')['image'][:]
...
def random_translate(image, label):
    ...
    image = np.roll(image, k, axis=a)
    label = np.roll(label, k, axis=a)
    return image, label
...
class RandomGenerator(object):
    ...
    def __call__(self, sample):
        ...
        if random.random() > 0.67:
            ...
            image, label = random_translate(image, label)
        x, y = image.shape
        image = zoom(image, (self.output_size[0] / x, self.output_size[1] / y), order=0)
        # image = np.repeat(np.expand_dims(image, axis=0), repeats=3, axis=0)
...
class RandomGenerator25D(object):
    def __init__(self, output_size):
        self.output_size = output_size
