...
def pre_train(args, snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            loss_dice = dice_loss(output_soft, label_batch.unsqueeze(1))
            loss_ce = CE(outputs, label_batch).mean()
...
def self_train(args ,pre_snapshot_path, snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            #-- supervised
            pre_u = model(uimg)
            pre_u_soft = F.softmax(pre_u, dim=1)
            plab = plab.type(torch.int64)
            loss_dice_u = dice_loss(pre_u_soft, plab.unsqueeze(1))
            loss_ce_u = CE(pre_u, plab).mean()
            
            loss_ce = loss_ce_l + loss_ce_u 
            loss_dice = loss_dice_l + loss_dice_u
...
if __name__ == "__main__":
    ...
    shutil.copy('code/ACDC_MT_train.py', self_snapshot_path)
