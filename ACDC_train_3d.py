...
from dataloaders.dataset import (BaseDataSets, RandomGenerator3D, TwoStreamBatchSampler, ThreeStreamBatchSampler)
from networks.Unet3D import UNet
...
parser.add_argument('--patch_size', type=list,  default=[256, 256, 16], help='patch size of network input')
...
def self_train(args ,pre_snapshot_path, snapshot_path):
    ...
    model = UNet(in_dim=1, out_dim=num_classes).cuda()
    ...
    db_train = BaseDataSets(base_dir=args.root_path,
                            ...
                            num=None,
                            num_d=3,
                            transform=transforms.Compose([RandomGenerator3D(args.patch_size)]))
    db_val = BaseDataSets(base_dir=args.root_path, split="val")

    trainloader = DataLoader(db_train, batch_size=args.batch_size, num_workers=4, pin_memory=True, worker_init_fn=worker_init_fn)
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            if iter_num > 0 and iter_num % 9 == 0:
                model.eval()
                metric_list = 0.0
                for _, sampled_batch in enumerate(valloader):
                    metric_i = test_3d_patch.test_single_volume(sampled_batch["image"], sampled_batch["label"], model, classes=num_classes)
